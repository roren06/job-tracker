...
const requireAuth_1 = require("../middleware/requireAuth"); // adjust if your path differs
const prisma_1 = require("../prisma"); // adjust if your prisma export path differs
 ...
 * }
 */
router.get("/usage/:applicationId", async (req, res) => {
    const applicationId = String(req.params.applicationId ?? "").trim();
    if (!applicationId)
        return res.status(400).json({ error: "Missing applicationId." });
    const count = await prisma_1.prisma.aiGeneration.count({
        where: { userId: req.userId, applicationId },
    });
    // optional: last generated timestamp
    const last = await prisma_1.prisma.aiGeneration.findFirst({
        where: { userId: req.userId, applicationId },
...
router.get("/history/:applicationId", async (req, res) => {
    ...
    const items = await prisma_1.prisma.aiGeneration.findMany({
        where: {
...
router.post("/generate", async (req, res) => {
    try {
        assertEnv();
        const { action, application, prompt } = req.body;
        if (!action)
            badRequest("Missing action.");
        const allowed = [
            "resume_bullet",
            "followup_email",
            "improve_notes",
        ...
        if (!applicationId)
            badRequest("Missing application.id.");
        // ✅ Security: make sure this application belongs to the logged-in user
        const owned = await prisma_1.prisma.application.findFirst({
        ...
        if (!company)
            badRequest("Missing application.company.");
        if (!role)
            badRequest("Missing application.role.");
        ...
        const actionSpec = {
            resume_bullet: "Write 1 strong, metrics-friendly resume bullet tailored to this role. Use action verb + impact. If no metrics, suggest a plausible metric placeholder like '(X%)' but label it as placeholder.",
            followup_email: "Draft a professional follow-up email after an application or interview. Keep it under 150 words. Include a subject line. Use a polite, confident tone.",
            improve_notes: "Rewrite the notes into clean, structured, skimmable bullet points. Preserve facts, remove fluff. If notes are empty, propose 5 note prompts to capture next.",
            interview_tips: "Provide 6 interview prep tips specific to the role, including 2 likely questions + what a good answer should cover.",
            cover_letter: "Write a concise, professional cover letter tailored to this role and company. Keep it under 250 words. Focus on relevant skills, impact, and motivation. Do not invent false experience.",
        };
        const input = [
            {
                role: "user",
        ...
        try {
            ...
            if (!applicationId) {
                ...
                await prisma_1.prisma.aiGeneration.create({
                    data: {
                        userId: req.userId,
                        applicationId,
                        action,
...
// ===== DEV FALLBACK (FREE, NO BILLING REQUIRED) =====
function mockAiResponse(action, company, role, notes, userPrompt) {
    ...
    switch (action) {
        case "resume_bullet":
            return applyStyle(`• Built scalable frontend features using React and async data patterns, improving UX responsiveness and maintainability (placeholder metric: +20%).`);
        case "followup_email":
            return applyStyle(`Subject: Follow-up on ${role} application
...
[Your Name]`);
        case "improve_notes":
            return applyStyle(`• Applied for ${role} at ${company}
...
• Review company tech stack before interview`);
        case "interview_tips":
            return applyStyle(`• Review React state management and async patterns
...
• Research the company's product and tech stack`);
        case "cover_letter":
            return applyStyle(`Dear ${company} Hiring Team,
